            ...
            if (Request::initial()->is_ajax())
            {
                ...
                $data=array();
                    if(!empty($photos))
                    {
                            ...
                            {       if(getimagesize($photo["path"])[0] >= getimagesize($photo["path"])[1])
                                    {
                                        $width=(getimagesize($photo["path"])[0]>=500) ? 500 : getimagesize($photo["path"])[0];
                                        $height= $width / ((getimagesize($photo["path"])[0])/(getimagesize($photo["path"])[1]));
                                        $data[]=array('path' => $photo["path"], 'height' => $height, 'width' => $width, 'side' => 'width', 'size' => $width);
                                    }
                                    else
                                    {
                                        $height=(getimagesize($photo["path"])[1]>=500) ? 500 : getimagesize($photo["path"])[1];
                                        $width= $height * ((getimagesize($photo["path"])[0])/(getimagesize($photo["path"])[1]));
                                        $data[]=array('path' => $photo["path"], 'width' => $width, 'height' => $height, 'side' => 'height', 'size' => $height);
                                    }
                            }
